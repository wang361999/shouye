 ...
 *
 * 优先级判断：
 *   1. 如果配置了 RESEND_API_KEY → 用 Resend（两个平台都可用）
 ...
 *
 * @param params 邮件内容
 * @param smtpConfig SMTP 配置（从数据库读取，可选）
...
export async function sendEmail(
  ...
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      return await sendViaResend(params, fromName);
    }

    // 2. 回退到 SMTP（仅 Vercel / Node.js 环境可用）
    if (smtpConfig && smtpConfig.smtp_host && smtpConfig.smtp_user) {
      return await sendViaSmtp(params, smtpConfig, fromName);
    }
    ...
    return {
      ...
      message:
        '邮件发送未配置。请选择一种方式：\n' +
        '方式一（推荐，双平台通用）：在环境变量中设置 RESEND_API_KEY 和 RESEND_FROM_EMAIL\n' +
